 ...
 * @param   {string}    `permission`  название разрешения, проверятся при автодополнении команды (необязательный параметр). Если параметр был указан а разрешение отсутствует, выполнение команды отменяется, а пользователю выводится сообщение об отсутствии прав на выполнение команды
 * @return  {object}    Экземпляр   класса Completer содержащий методы для добавления дополнительных параметров к команде их обработчиков и функций автодополнения

`addConsoleCommand(comandName, handler, completer, permission)` -  производит регистрацию псевдонима команды как серверной (работает из консоли сервера).
 * @param   {string}    `comandName`  название псевдонима. например "/warp"
 * @param   {function}  `handler`     функция-обработчик для данного участка команды (необязательный параметр)
 * @param   {function}  `completer`   функция формирующая дополнительный список автодополнений для следующего участка команды (необязательный параметр)
 * @param   {string}    `permission`  название разрешения, проверятся при автодополнении команды (необязательный параметр). Если параметр был указан а разрешение отсутствует, выполнение команды отменяется, а пользователю выводится сообщение об отсутствии прав на выполнение команды
...
`addPlayerCommand(comandName, handler, completer, permission)` -  производит регистрацию псевдонима команды как клиентской (работает из чата игры).
 * @param   {string}    `comandName`  название псевдонима. например "/warp"
 * @param   {function}  `handler`     функция-обработчик для данного участка команды (необязательный параметр)
 * @param   {function}  `completer`   функция формирующая дополнительный список автодополнений для следующего участка команды (необязательный параметр)
 * @param   {string}    `permission`  название разрешения, проверятся при автодополнении команды (необязательный параметр). Если параметр был указан а разрешение отсутствует, выполнение команды отменяется, а пользователю выводится сообщение об отсутствии прав на выполнение команды
...
`addComplete(complateName,handler,completer,permission)` - Метод класса Completer. Добавляет к команде параметр и его обработчик.
 * @param      {string}     `complateName`  наименование добавляемой команды/параметра.
...
`setHandler(handler)` - Метод класса Completer.  Устанавливает (заменяет если уже установлен) обработчик для текущего объекта класса Completer.
 * @param      {function}   `handler`       обработчик который будет вызван при вводе этой команды (не обязательный параметр).
 
`setCompleter(completer)` - Метод класса Completer. Устанавливает (заменяет если уже установлена) функцию, результат выполнения которой, ассоциативный массив. Его ключи будет добавлены к списку автодополнения текущей команды, представленной текущим объектом класса  Completer (не обязательный параметр).
 * @param      {function}   `completer`       функция добавляющая элементы к списку автозавершения (не обязательный параметр).
